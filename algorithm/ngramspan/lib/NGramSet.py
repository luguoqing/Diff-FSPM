...
class NGramSet(Counter):
    ...
    '''
    def __init__(self, max_len, N_max, min_sup):
        '''
    ...
    def load_dataset(self, in_file, dump_file):
        ...
        if not os.path.isfile(dump_file):
            print "File (%s) does not exist!"%(dump_file) 
            print "Creating File (%s)" % (dump_file)
    ...
    def load_dump(self, filename):
        ...
        print "ngrams total : %d" % (ngram_num)
        print "Loading ngrams file (%s, n_max=%d)..." % (filename, self.N_max)
        ...
        print "Alphabet size : %d" % (self.alphabet_size)
    ...
    def parse_sequences(self, filename):
        ...
        # be the end point
        self.TERM = self.alphabet_size
        print "Alphabet size : %s" % (str(self.alphabet_size)) 
        print "Termination code : %s" % (str(self.TERM))
        print "Number of sequences : (%s)" % (self.all_record_num)
